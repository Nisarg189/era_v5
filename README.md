...
Percentages below are of that stage, not of the whole run.

| Stage | Span | Tokens | web | code | indic | stem | reason | longctx | agentic |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| S0 seed | 2% | 0.30T | 52 | 14 | 15 | 10 | 3 | 2 | 4 |
| S1 general | 32% | 4.80T | 46 | 20 | 14 | 11 | 4 | 2 | 3 |
| S2 capability | 32% | 4.80T | 30 | 31 | 12 | 12 | 8 | 4 | 3 |
| S3 reasoning | 18% | 2.70T | 22 | 26 | 12 | 9 | 17 | 11 | 3 |
| S4 long context | 13% | 1.95T | 18 | 24 | 12 | 6 | 11 | 26 | 3 |
| S5 anneal | 3% | 0.45T | 8 | 16 | 26 | 10 | 18 | 10 | 12 |
...
| Indic | 0.20T | acquire | licensed news archives, book and textbook digitisation, broadcast transcription, government and court records |
| Indic | 0.185T | generate | verified translation into tier C, templated and self-instruct text into tier D |
...
By mechanism: 0.60T packed, 0.46T generated, 0.35T mined, 0.30T acquired. **Only 0.46T
of the 1.75T is model-generated.** The rest is repackaged from data the programme
already holds, or bought and digitised. That ratio is deliberate. Synthetic text is the
...
**The most consequential line in this plan is the 200 billion tokens of newly digitised
native text.** Using only what exists today, tier A supplies 256B tokens even at four
passes, which is 13 percent of the slot. Everything else would have to be filled with
translated or generated text until roughly 60 percent of the Indic lane was
...
For an India-first model, **acquisition is the lever and generation is the fallback**,
and that acquisition is a funded programme with scanners, licences and staff, not a
data-processing step.

Quality gates on the generated tiers: tier C is accepted only above a fixed round-trip
translation agreement threshold, tier D only after native-speaker sampling at one
document in ten thousand. Tier A is never diluted with either and stays separately
...
**How results are measured.** Held-out **bits per byte**, not loss. Devanagari costs
about 8.3 bytes per token in this vocabulary against 3.7 for English, so a per-token
loss would compare scripts rather than models. Bits per byte removes the tokenizer from
the comparison. Absolute values are still not comparable *between* lanes, because
...
The same model trained five times, varying only the Indic share.
...
Restated as what each increment purchases and what it costs the other three lanes:
...
## 17 What would refute this plan

The remaining open questions are specified at 1B and 3B, with the decision rule fixed
in advance so no result can be reinterpreted once it arrives.
